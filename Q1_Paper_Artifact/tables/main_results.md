# Main Results Table (manuscript Table 3, full version)

**Source**: `results/P0_device_gap`, 1,120 completed fold-runs, averaged over 4 classifiers and applicable folds/protocols. Full per-run data: `MASTER_RESULTS.csv` (`experiment_family == "main_benchmark"`).

| Representation | Regime | Balanced Accuracy | F1 | ROC-AUC | MCC |
|---|---|---|---|---|---|
...
| odi_hb (degenerate) | cross | 0.4958 | 0.4622 | 0.4949 | −0.0095 |
| odi_hb (degenerate) | matched | 0.4958 | 0.4622 | 0.4949 | −0.0095 |
...
**Note.** `odi_hb` alone is a per-subject-constant feature and is chance-level by construction for a per-window classifier (Section 6.5) — retained in this table for completeness, excluded from the generalization-gap ranking (Figure 6) as a degenerate baseline.
